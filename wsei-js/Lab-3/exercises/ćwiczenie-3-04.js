...
class Tools {
  name(name) {
    return name && name.length <= 25 ? true : false;
  }
  ects(ects) {
    return (
      Number.isInteger(Number(ects)) && Number(ects) > 0 && Number(ects) <= 200
    );
  }
  active(active) {
    return (
  ...
  statusToBool(status) {
    if (status === "aktywna" || status === "aktywny") return true;

    if (status === "nieaktywna" || status === "nieaktywny") return false;
  }
  toObject(array) {
    return {
      name: array[0],
      ects: Number(array[1]),
      active: this.statusToBool(array[2]),
    };
  }
}
...
function parseStudents(input) {
  ...
  let students = input
    .split("\n")
    .filter((student) => student.split("\t").length === 3)
    .map((student) => student.split("\t"))
    .filter((student) => tools.name(student[0]))
    .filter((student) => tools.ects(student[1]))
    .filter((student) => tools.active(student[2]))
    .map((student) => tools.toObject(student));
  return students;
}
